...
- **Legend filtering runs through one function.** The Builders and Lot Status checkboxes both fade lots, and both resolve in a single `applyLotFilters()` pass — a lot dims if *either* filter excludes it. `applyBuilderFilter()`/`applyStatusFilter()` are thin wrappers kept for the inline `onchange` handlers and the other call sites. **Don't give either filter its own `.dimmed` pass**: they target the same elements, so two independent passes clobber each other (unchecking a builder and then a status used to undo the builder's dimming). A dimmed lot fades as a whole — `.lot`, `.lot-stroke`, `.builder-dot`, and `.lot-label-group` all take `.dimmed` together, since fading only the builder band is invisible against a same-colored lot fill.
